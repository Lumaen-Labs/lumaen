...
// // Ensure you use AccountInfo::resize() instead of AccountInfo::realloc() if resizing is needed
...
//     pub fn renounce_role(ctx: Context<RenounceRole>,role:u8) -> Result<()> {
//         emit!(RoleRenounced {
//             role: ctx.accounts.role_account.role.clone(),
...
//     #[account(
//         seeds = [b"role", registry.key().as_ref(), role.as_(), account.key().as_ref()],
//         bump
//     )]
//     pub role_account: Account<'info, RoleAccount>,
// }
